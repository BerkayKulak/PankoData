...
class SicaklikToprakNemGrafigiBar(QtWidgets.QMainWindow):

    def __init__(self, *args, **kwargs):
        super(SicaklikToprakNemGrafigiBar, self).__init__(*args, **kwargs)
        self.setGeometry(600, 350, 600, 450)
        self.graphWidget = pg.PlotWidget()
        self.setCentralWidget(self.graphWidget)
        self.setWindowTitle("Sıcaklık ve Harcanan Su grafikleri")
        ...
        ax.bar(dataframe.index, dataframe["sicaklik"])
        ax2.bar(dataframe2.index, dataframe2["nem"], color="red")
        plt.show()
...
def ass():
    ...
    ass = SicaklikToprakNemGrafigiBar()
    ass.show()
    sys.exit(app.exec_())
